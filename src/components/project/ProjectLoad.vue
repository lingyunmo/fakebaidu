...
const uploadedCount = ref(0); // 已上传的项目数量
const totalAssignments = 86; // 总项目数量

onMounted(async () => {
  try {
    // 从后端获取项目状态
    const response = await fetch('/api/status/projectNowCheck');
    if (response.ok) {
      const data = await response.json();
      uploadedCount.value = data["uploadCount"]; // 返回上传数量
    } else {
      console.error('获取项目状态失败:', response.statusText);
    }
  } catch (error) {
...
.progress-wrapper {
  position: relative;
  height: 30px;
  background-color: #003366; /* 深蓝色进度条背景 */
...
.progress-bar {
  height: 100%;
  background: linear-gradient(270deg, rgba(0, 191, 255, 1), rgba(0, 191, 255, 0.5));
